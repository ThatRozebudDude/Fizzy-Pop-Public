...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		var bg:FlxSprite = new FlxSprite(-80).loadGraphic(Paths.image('menuBG2'));
		bg.scrollFactor.x = 0;
		...
		magenta = new FlxSprite(-80).loadGraphic(Paths.image('menuBGMagenta2'));
		magenta.scrollFactor.x = 0;
		magenta.scrollFactor.y = 0.18;
		magenta.setGraphicSize(Std.int(magenta.width * 2.1));
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (controls.ACCEPT)
			{
				...
				{
					#if linux
					Sys.command('/usr/bin/xdg-open', ["https://ko-fi.com/pipflippp", "&"]);
					#else
					FlxG.openURL('https://ko-fi.com/pipflippp');
					#end
				}
